...
const s={files:[],selectedId:null,outputDirectory:'',platform:'windows',locale:'en',isDragOverlayVisible:false,previewCache:{},previewToken:0,previewImage:null,previewCropRect:null,watermarks:[],overlays:[],interaction:null};
...
const overlayModel=(canvasW,canvasH,overlayImg,o)=>{
    ...
    let ox,oy,ow=Math.max(1,Math.round(canvasW*(o.scale/100)));
...
const renderOverlayList=()=>{
    ...
    s.overlays.forEach((ov,i)=>{
        const item=document.createElement('div'); item.className='layer-item';
        item.innerHTML=`<div class="layer-item-header"><span>${ov.name}</span><button class="remove-layer-btn" data-type="ov" data-id="${ov.id}">×</button></div><div class="layer-body"><label class="switch"><input id="ov-enabled-${i}" type="checkbox" ${ov.enabled?'checked':''} /><span>${tt('enableThis')}</span></label><div class="grid two"><label><span>X</span><input id="ov-x-${i}" type="number" value="${ov.x??''}" /></label><label><span>Y</span><input id="ov-y-${i}" type="number" value="${ov.y??''}" /></label></div><div class="grid three"><label><span>${tt('scalePercent')}</span><input id="ov-scale-${i}" type="number" value="${ov.scale}" /></label><label><span>${tt('opacity')}</span><input id="ov-opacity-${i}" type="number" step="0.05" value="${ov.opacity}" /></label><label><span>${tt('margin')}</span><input id="ov-margin-${i}" type="number" value="${ov.margin}" /></label></div><label><span>${tt('position')}</span><select id="ov-pos-${i}"><option value="top-left" ${ov.position==='top-left'?'selected':''}>${tt('topLeft')}</option><option value="top-center" ${ov.position==='top-center'?'selected':''}>${tt('topCenter')}</option><option value="top-right" ${ov.position==='top-right'?'selected':''}>${tt('topRight')}</option><option value="center-center" ${ov.position==='center-center'?'selected':''}>${tt('center')}</option><option value="bottom-left" ${ov.position==='bottom-left'?'selected':''}>${tt('bottomLeft')}</option><option value="bottom-center" ${ov.position==='bottom-center'?'selected':''}>${tt('bottomCenter')}</option><option value="bottom-right" ${ov.position==='bottom-right'?'selected':''}>${tt('bottomRight')}</option></select></label></div></div>`;
...
const addWatermark=()=>{ s.watermarks.push({id:Date.now(),text:'Watermark',fontSize:40,opacity:0.5,color:'#ffffff',x:null,y:null,margin:20,position:'bottom-right',enabled:true}); renderWatermarkList(); schedule(0); };
const addOverlay=async()=>{ try{const file=await cmd('pick_overlay');if(!file)return;s.overlays.push({id:Date.now(),path:file.path,name:file.name,scale:25,opacity:0.8,x:null,y:null,margin:20,position:'center-center',enabled:true,width:file.width,height:file.height});renderOverlayList();schedule(0);}catch(err){console.error(err);}};
const renderFiles=()=>{
...
async function buildWatermarkData(){
    ...
    wms.forEach(wm=>{
        ...
        ctx.save(); ctx.font=`600 ${model.fontSize}px sans-serif`; ctx.textAlign='left'; ctx.textBaseline='top';
...
function onPreviewMove(event){
    ...
    if(s.interaction.type.includes('move')){
        const r=s.interaction.rect,nx=r.x+dx,ny=r.y+dy;
        ...
        if(s.interaction.type==='crop-resize') syncCrop({x:nx,y:ny,width:nw,height:nh});
            ...
            if(item.fontSize!==undefined){
                ...
                item.scale=Math.round((nw/cw)*100);
                if(document.getElementById(`ov-scale-${s.interaction.index}`)) document.getElementById(`ov-scale-${s.interaction.index}`).value=item.scale;
            }
            if(document.getElementById(`${pre}-x-${s.interaction.index}`)) document.getElementById(`${pre}-x-${s.interaction.index}`).value=Math.round(nx);
            if(document.getElementById(`${pre}-y-${s.interaction.index}`)) document.getElementById(`${pre}-y-${s.interaction.index}`).value=Math.round(ny);
        }
