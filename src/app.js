...
/* 多语言文案表。 */
const tr={en:{appTitle:'EZCut',appSubtitle:'Batch image editor',previewEyebrow:'Preview',previewTitle:'Image Preview',addImages:'Add Images',dropHere:'Drop images here',dropSupport:'PNG, JPG, WEBP, TIFF, BMP, GIF',noImageSelected:'No image selected',libraryEyebrow:'Library',imagesTitle:'Images',noImagesYet:'Drag photos here',settingsEyebrow:'Settings',outputSettings:'Output Settings',resize:'Resize',crop:'Crop',textWatermark:'Text Watermark',overlayImage:'Overlay Image',filename:'Filename',export:'Export',width:'Width',height:'Height',mode:'Mode',fit:'Fit',fill:'Fill',stretch:'Stretch',enableThis:'Enable this section',text:'Text',size:'Size',opacity:'Opacity',margin:'Margin',color:'Color',position:'Position',topLeft:'Top Left',topCenter:'Top Center',topRight:'Top Right',centerLeft:'Left',center:'Center',centerRight:'Right',bottomLeft:'Bottom Left',bottomCenter:'Bottom',bottomRight:'Bottom Right',choose:'Choose',noOverlaySelected:'No overlay selected',scalePercent:'Scale %',template:'Template',exportCategory:'Export Category',keepOriginal:'Keep original',useRegexRename:'Use regex rename',pattern:'Pattern',replace:'Replace',flags:'Flags',startNumber:'Start Number',step:'Step',chooseSaveFolder:'Choose Save Folder',defaultSaveFolder:'Default: save next to each source image',defaultSaveFolderRuntime:'Default: saved next to each source image',clearAfterExport:'Clear image list after export',ready:'Ready',remove:'Remove',loadedCount:'Loaded {count} image(s)',overlaySelected:'Overlay selected',customSaveFolderUpdated:'Custom save folder updated',customSaveFolder:'Custom save folder: {path}',processing:'Processing...',tauriUnavailable:'Tauri API not available',addAtLeastOne:'Please add at least one image',doneSummary:'Done: {ok} exported, {fail} failed',processingFailed:'Processing failed: {error}',droppedCount:'Dropped {count} image(s)',remainingCount:'Remaining {count} image(s)',saved:'Saved: {path}',failed:'Failed: {path} - {message}',selectedMeta:'{name} · {width} x {height} · {size}',placeholderLeaveEmpty:'Leave empty',placeholderOriginalWidth:'Original width',placeholderOriginalHeight:'Original height',placeholderWatermark:'Watermark text',placeholderRegex:'^IMG_',placeholderReplace:'product_',close:'Close',regexHelpTitle:'Filename Regex Guide',addText:'+ Add Text',addImage:'+ Add Image',layerX:'X Coord',layerY:'Y Coord',fontFamily:'Font Family',regexHelpContent:'<h4>Placeholders</h4><ul><li><code>{name}</code>: Refers to the filename (processed by regex if enabled)</li><li><code>{index}</code>: Sequential number (customizable via Start/Step)</li><li><code>{timestamp}</code>: Current time (YYYYMMDD_HHMMSS)</li><li><code>{width}</code> / <code>{height}</code>: Original image dimensions</li><li><code>{ext}</code>: Target file extension</li></ul><h4>Regex Rename</h4><ul><li><code>^</code> / <code>$</code>: Start/End of line</li><li><code>(.*)</code>: Capture groups, use <code>$1</code>, <code>$2</code> in Replace field to refer to these parts.</li></ul><h4>Flags</h4><ul><li><code>g</code>: Global match</li><li><code>i</code>: Case insensitive</li><li><code>m</code>: Multiline</li><li><code>s</code>: Dot matches newline</li></ul><h4>Sequence Number</h4><ul><li>Use <b>Start Number</b> to set the beginning of the sequence.</li><li>Use <b>Step</b> to define the increment for the next file.</li></ul>'},'zh-CN':{appTitle:'EZCut',appSubtitle:'批量图片编辑器',previewEyebrow:'预览',previewTitle:'图片预览',addImages:'添加图片',dropHere:'将图片拖到这里',dropSupport:'支持 PNG、JPG、WEBP、TIFF、BMP、GIF',noImageSelected:'未选择图片',libraryEyebrow:'列表',imagesTitle:'图片列表',noImagesYet:'拖动照片到此处',settingsEyebrow:'设置',outputSettings:'输出设置',resize:'修改尺寸',crop:'裁剪',textWatermark:'文字水印',overlayImage:'图层覆盖',filename:'文件名',export:'导出',width:'宽度',height:'高度',mode:'模式',fit:'等比适配',fill:'铺满裁切',stretch:'强制拉伸',enableThis:'启用此分组',text:'文字',size:'大小',opacity:'透明度',margin:'边距',color:'颜色',position:'位置',topLeft:'左上',topCenter:'上中',topRight:'右上',centerLeft:'左中',center:'居中',centerRight:'右中',bottomLeft:'左下',bottomCenter:'下中',bottomRight:'右下',choose:'选择',noOverlaySelected:'未选择覆盖图层',scalePercent:'缩放 %',template:'模板',exportCategory:'导出分类',keepOriginal:'保持原格式',useRegexRename:'启用正则改名',pattern:'正则',replace:'替换',flags:'标志',startNumber:'起始序号',step:'步进',chooseSaveFolder:'选择保存目录',defaultSaveFolder:'默认保存到每张原图所在目录',defaultSaveFolderRuntime:'默认保存到每张原图所在目录',clearAfterExport:'导出后清空图片列表',ready:'就绪',remove:'移除',loadedCount:'已载入 {count} 张图片',overlaySelected:'已选择覆盖图层',customSaveFolderUpdated:'已更新自定义保存目录',customSaveFolder:'自定义保存目录：{path}',processing:'正在处理...',tauriUnavailable:'Tauri 接口不可用',addAtLeastOne:'请至少添加一张图片',doneSummary:'处理完成：成功 {ok} 张，失败 {fail} 张',processingFailed:'处理失败：{error}',droppedCount:'已拖入 {count} 张图片',remainingCount:'剩余 {count} 张图片',saved:'已保存：{path}',failed:'失败：{path} - {message}',selectedMeta:'{name} · {width} x {height} · {size}',placeholderLeaveEmpty:'留空则不限制',placeholderOriginalWidth:'原图宽度',placeholderOriginalHeight:'原图高度',placeholderWatermark:'输入水印文字',placeholderRegex:'^IMG_',placeholderReplace:'product_',close:'关闭',regexHelpTitle:'文件名手册',addText:'+ 添加文字',addImage:'+ 添加图片',layerX:'X 坐标',layerY:'Y 坐标',fontFamily:'字体',regexHelpContent:'<h4>占位符规则</h4><ul><li><code>{name}</code>：原文件名（若启用正则，指代正则处理后的名字）</li><li><code>{index}</code>：序列号（由下方“起始序号”和“步进”控制）</li><li><code>{timestamp}</code>：当前时间戳 (YYYYMMDD_HHMMSS)</li><li><code>{width}</code> / <code>{height}</code>：原图的分辨率宽高</li><li><code>{ext}</code>：目标文件的扩展名</li></ul><h4>正则命名</h4><ul><li><code>^</code> / <code>$</code>：匹配开头/结尾</li><li><code>(.*)</code>：捕获组。在“替换”栏中使用 <code>$1</code>, <code>$2</code> 引用这些部分。</li></ul><h4>标志 (Flags)</h4><ul><li><code>g</code>：全局匹配</li><li><code>i</code>：忽略大小写</li><li><code>m</code>：多行模式</li><li><code>s</code>：允许 . 匹配所有字符（包括换行）</li></ul><h4>序号序列</h4><ul><li><b>起始序号</b>：首张图片的开始数字。</li><li><b>步进</b>：后续每张图片序号递加的数值。</li></ul>'},'zh-TW':{appTitle:'EZCut',appSubtitle:'批量圖片編輯器',previewEyebrow:'預覽',previewTitle:'圖片預覽',addImages:'新增圖片',dropHere:'將圖片拖曳至此',dropSupport:'支援 PNG、JPG、WEBP、TIFF、BMP、GIF',noImageSelected:'未選擇圖片',libraryEyebrow:'列表',imagesTitle:'圖片清單',noImagesYet:'拖動照片到此處',settingsEyebrow:'設定',outputSettings:'輸出設定',resize:'調整大小',crop:'裁剪',textWatermark:'文字浮水印',overlayImage:'圖層疊加',filename:'檔案名稱',export:'匯出',width:'寬度',height:'高度',mode:'模式',fit:'等比縮放',fill:'鋪滿裁切',stretch:'強制拉伸',enableThis:'啟用此分組',text:'文字',size:'大小',opacity:'不透明度',margin:'邊距',color:'顏色',position:'位置',topLeft:'左上',topCenter:'上中',topRight:'右上',centerLeft:'左中',center:'居中',centerRight:'右中',bottomLeft:'左下',bottomCenter:'下中',bottomRight:'右下',choose:'選擇',noOverlaySelected:'未選擇疊加圖層',scalePercent:'縮放 %',template:'模板',exportCategory:'匯出分類',keepOriginal:'保留原始格式',useRegexRename:'使用正則更名',pattern:'正則',replace:'替換',flags:'標記',startNumber:'起始序號',step:'步進',chooseSaveFolder:'選擇儲存目錄',defaultSaveFolder:'預設儲存到每張原圖所在目錄',defaultSaveFolderRuntime:'預設儲存到每張原圖所在目錄',clearAfterExport:'匯出後清空清單',ready:'就緒',remove:'移除',loadedCount:'已載入 {count} 張圖片',overlaySelected:'已選擇疊加圖層',customSaveFolderUpdated:'已更新自定義儲存目錄',customSaveFolder:'自定義儲存目錄：{path}',processing:'處理中...',tauriUnavailable:'Tauri 接口不可用',addAtLeastOne:'請至少新增一張圖片',doneSummary:'處理完成：成功 {ok} 張，失敗 {fail} 張',processingFailed:'處理失敗：{error}',droppedCount:'已拖入 {count} 個檔案',remainingCount:'剩餘 {count} 張',saved:'已儲存：{path}',failed:'失敗：{path} - {message}',selectedMeta:'{name} · {width} x {height} · {size}',placeholderLeaveEmpty:'留空不限制',placeholderOriginalWidth:'原始寬度',placeholderOriginalHeight:'原始高度',placeholderWatermark:'輸入浮水印文字',placeholderRegex:'^IMG_',placeholderReplace:'product_',close:'關閉',regexHelpTitle:'檔案名稱正則手冊',addText:'+ 新增文字',addImage:'+ 新增圖片',layerX:'X 座標',layerY:'Y 座標',fontFamily:'字體',regexHelpContent:'<h4>佔位符規則</h4><ul><li><code>{name}</code>：原檔案名稱</li><li><code>{index}</code>：序號（可自定義起始與步進）</li><li><code>{timestamp}</code>：當前時間戳</li><li><code>{width}</code> / <code>{height}</code>：原圖分辨率</li></ul><h4>正則命名</h4><ul><li><code>^</code> / <code>$</code>：匹配開頭/結尾</li><li>捕獲組：在「替換」欄中使用 <code>$1</code>, <code>$2</code>。</li></ul><h4>標誌 (Flags)</h4><ul><li><code>g</code>：全局匹配</li><li><code>i</code>：忽略大小写</li><li><code>m</code>：多行模式</li></ul>'},'ja':{appTitle:'EZCut',appSubtitle:'一括画像エディタ',previewEyebrow:'プレビュー',previewTitle:'画像プレビュー',addImages:'画像を追加',dropHere:'ここに画像をドロップ',dropSupport:'PNG, JPG, WEBP, TIFF, BMP, GIF 対応',noImageSelected:'画像が選択されていません',libraryEyebrow:'ライブラリ',imagesTitle:'画像リスト',noImagesYet:'ここに写真をドロップ',settingsEyebrow:'設定',outputSettings:'出力設定',resize:'サイズ変更',crop:'切り抜き',textWatermark:'テキスト透かし',overlayImage:'画像オーバーレイ',filename:'ファイル名',export:'エクスポート',width:'幅',height:'高さ',mode:'モード',fit:'フィット',fill:'塗りつぶし',stretch:'ストレッチ',enableThis:'有効化',text:'テキスト',size:'サイズ',opacity:'不透明度',margin:'マージン',color:'色',position:'位置',topLeft:'左上',topCenter:'上中央',topRight:'右上',centerLeft:'左中央',center:'中央',centerRight:'右中央',bottomLeft:'左下',bottomCenter:'下中央',bottomRight:'右下',choose:'選択',noOverlaySelected:'未選択',scalePercent:'スケール %',template:'テンプレート',exportCategory:'エクスポート',keepOriginal:'オリジナルを保持',useRegexRename:'正規表現で変更',pattern:'パターン',replace:'置換',flags:'フラグ',startNumber:'開始番号',step:'ステップ',chooseSaveFolder:'保存先を選択',defaultSaveFolder:'デフォルト：元の場所',defaultSaveFolderRuntime:'デフォルト：元の場所',clearAfterExport:'完了後にリストをクリア',ready:'準備完了',remove:'削除',loadedCount:'{count} 枚読み込み済み',overlaySelected:'オーバーレイ選択済み',customSaveFolderUpdated:'保存先を更新しました',customSaveFolder:'保存先：{path}',processing:'処理中...',tauriUnavailable:'Tauri 使用不可',addAtLeastOne:'画像を追加してください',doneSummary:'完了：成功 {ok}, 失敗 {fail}',processingFailed:'失敗：{error}',droppedCount:'{count} 個ドロップ',remainingCount:'残り {count} 枚',saved:'保存：{path}',failed:'失敗：{path} - {message}',selectedMeta:'{name} · {width} x {height} · {size}',placeholderLeaveEmpty:'空のままにする',placeholderOriginalWidth:'元の幅',placeholderOriginalHeight:'元の高さ',placeholderWatermark:'テキストを入力',placeholderRegex:'^IMG_',placeholderReplace:'product_',close:'閉じる',regexHelpTitle:'正規表現ヘルプ',addText:'+ テキスト追加',addImage:'+ 画像追加',layerX:'X 座標',layerY:'Y 座標',fontFamily:'フォント',regexHelpContent:'<h4>プレースホルダー</h4><ul><li><code>{name}</code>：元のファイル名</li><li><code>{index}</code>：通し番号（開始番号とステップを変更可能）</li><li><code>{timestamp}</code>：現在時刻</li></ul><h4>正規表現</h4><ul><li><code>(.*)</code> でキャプチャし、置換で <code>$1</code> などを使用します。</li><li>フラグ: <code>g</code>(全体)、<code>i</code>(大文字小文字無視)など</li></ul>'},'ko':{appTitle:'EZCut',appSubtitle:'일괄 이미지 편집기',previewEyebrow:'미리보기',previewTitle:'이미지 미리보기',addImages:'이미지 추가',dropHere:'여기에 이미지 놓기',dropSupport:'PNG, JPG, WEBP, TIFF, BMP, GIF 지원',noImageSelected:'선택된 이미지 없음',libraryEyebrow:'라이브러리',imagesTitle:'이미지 목록',noImagesYet:'여기에 사진을 드래그하세요',settingsEyebrow:'설정',outputSettings:'출력 설정',resize:'크기 조절',crop:'자르기',textWatermark:'텍스트 워터마크',overlayImage:'이미지 오버레이',filename:'파일명',export:'내보내기',width:'너비',height:'높이',mode:'모드',fit:'맞춤',fill:'채우기',stretch:'늘리기',enableThis:'사용',text:'텍스트',size:'크기',opacity:'불투명도',margin:'여백',color:'색상',position:'위치',topLeft:'왼쪽 상단',topCenter:'상단 중앙',topRight:'오른쪽 상단',centerLeft:'왼쪽 중앙',center:'중앙',centerRight:'오른쪽 중앙',bottomLeft:'왼쪽 하단',bottomCenter:'하단 중앙',bottomRight:'오른쪽 하단',choose:'선택',noOverlaySelected:'선택 안 함',scalePercent:'비율 %',template:'템플릿',exportCategory:'내보내기',keepOriginal:'원본 유지',useRegexRename:'정규식 이름 변경',pattern:'패턴',replace:'교체',flags:'플래그',startNumber:'시작 번호',step:'증감',chooseSaveFolder:'저장 폴더 선택',defaultSaveFolder:'기본: 원본 위치',defaultSaveFolderRuntime:'기본: 원본 위치',clearAfterExport:'완료 후 목록 비우기',ready:'준비 완료',remove:'제거',loadedCount:'{count}개 로드됨',overlaySelected:'오버레이 선택됨',customSaveFolderUpdated:'저장 폴더 업데이트됨',customSaveFolder:'저장 폴더: {path}',processing:'처리 중...',tauriUnavailable:'Tauri 사용 불가',addAtLeastOne:'이미지를 추가하세요',doneSummary:'완료: 성공 {ok}, 실패 {fail}',processingFailed:'실패: {error}',droppedCount:'{count}개 드롭됨',remainingCount:'남은 항목 {count}개',saved:'저장: {path}',failed:'실패: {path} - {message}',selectedMeta:'{name} · {width} x {height} · {size}',placeholderLeaveEmpty:'비워두기',placeholderOriginalWidth:'원본 너비',placeholderOriginalHeight:'원본 높이',placeholderWatermark:'텍스트 입력',placeholderRegex:'^IMG_',placeholderReplace:'product_',close:'닫기',regexHelpTitle:'정규식 도움말',addText:'+ 텍스트 추가',addImage:'+ 이미지 추가',layerX:'X 좌표',layerY:'Y 좌표',fontFamily:'글꼴',regexHelpContent:'<h4>자리 표시자</h4><ul><li><code>{name}</code>: 원본 파일명</li><li><code>{index}</code>: 순번 (시작 번호 및 증감 값 설정 가능)</li><li><code>{timestamp}</code>: 현재 시각</li></ul><h4>플래그 (Flags)</h4><ul><li><code>g</code>: 전체 검색</li><li><code>i</code>: 대소문자 무시</li></ul>'}};

/* 运行期状态：增加对多图层（watermarks, overlays）的支持。 */
/* 
 * EZCut 前端逻辑核心 (Pure JS 版本)
 * 职责：管理 UI 状态、处理用户交互、调用 Tauri 后端指令以及维护预览画布。
 */
...
const tt=(k,vars={})=>((tr[s.locale]||tr.en)[k]||tr.en[k]||k).replace(/\{(\w+)\}/g,(match,t)=>t in vars?String(vars[t]):match);
...
const aboutText=(locale)=>{
    const table={
        en:{button:'About',title:'About EZCut',version:'Version',copyright:'Copyright © 2026 Kaiyuan. All rights reserved.',project:'Project',check:'Check Updates',sponsor:'Sponsor',idle:'Update status: idle',checking:'Checking updates...',latest:'You are on the latest version',available:'New version available: {version}',failed:'Update check failed'},
        'zh-CN':{button:'关于',title:'关于 EZCut',version:'版本',copyright:'版权所有 © 2026 Kaiyuan。保留所有权利。',project:'项目地址',check:'检测更新',sponsor:'赞助',idle:'更新状态：未检测',checking:'正在检测更新...',latest:'当前已是最新版本',available:'发现新版本：{version}',failed:'检测更新失败'},
        'zh-TW':{button:'關於',title:'關於 EZCut',version:'版本',copyright:'版權所有 © 2026 Kaiyuan。保留所有權利。',project:'專案地址',check:'檢查更新',sponsor:'贊助',idle:'更新狀態：未檢查',checking:'正在檢查更新...',latest:'目前已是最新版本',available:'發現新版本：{version}',failed:'檢查更新失敗'},
        ja:{button:'情報',title:'EZCut について',version:'バージョン',copyright:'Copyright © 2026 Kaiyuan. All rights reserved.',project:'プロジェクト',check:'更新を確認',sponsor:'スポンサー',idle:'更新状態: 未確認',checking:'更新を確認中...',latest:'最新バージョンです',available:'新しいバージョン: {version}',failed:'更新確認に失敗しました'},
        ko:{button:'정보',title:'EZCut 정보',version:'버전',copyright:'Copyright © 2026 Kaiyuan. All rights reserved.',project:'프로젝트',check:'업데이트 확인',sponsor:'후원',idle:'업데이트 상태: 미확인',checking:'업데이트 확인 중...',latest:'최신 버전입니다',available:'새 버전: {version}',failed:'업데이트 확인 실패'}
    };
...
const renderAboutInfo=()=>{
    if(!e.aboutModal) return;
    const t=aboutText(s.locale);
    if(e.aboutButton){e.aboutButton.title=t.button;e.aboutButton.setAttribute('aria-label',t.button);}
    if(e.aboutTitle)e.aboutTitle.textContent=t.title;
...
const hydrateAppVersion=async()=>{
    ...
    try{
        ...
        if(info?.app_version){
            s.appVersion=String(info.app_version);
            renderAboutInfo();
        }
    }catch(_){}
};
...
const renderFiles=()=>{
    ...
    if(s.files.length===0){s.selectedId=null;e.imageList.classList.add('empty-state');e.imageList.innerHTML=`<div class="empty-state-content"><p>${tt('noImagesYet')}</p></div>`;schedule(10);return;}
...
const getPayload=(watermarks)=>({
    ...
    naming:{
        ...
        regex_replacement:val('regex-replacement'),
        regex_flags:val('regex-flags'),
        start_number: Math.round(num('output-start')??1),
        step: Math.round(num('output-step')??1)
    },
    watermarks,
...
if(e.aboutButton&&e.aboutModal){
    e.aboutButton.addEventListener('click',()=>{renderAboutInfo();e.aboutModal.classList.add('is-visible');});
    e.aboutCloseButtons.forEach((b)=>b.addEventListener('click',()=>e.aboutModal.classList.remove('is-visible')));
    e.aboutModal.querySelector('.modal-backdrop')?.addEventListener('click',()=>e.aboutModal.classList.remove('is-visible'));
    e.checkUpdateButton?.addEventListener('click',checkForUpdates);
}
